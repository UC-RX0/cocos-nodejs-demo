      ...
      _export("BehaviorSource", BehaviorSource = class BehaviorSource {
        ...
        parse(content) {
          ...
          try {
            ...
            var postOrderNodes = postOrder(root); // 节点id和对象的映射

            var nodeIdInstanceMap = new Map(); // 倒序遍历

            for (var i = 0; i < postOrderNodes.length; i++) {
              var _node$children;

              var _node = postOrderNodes[i]; // 获取节点对应的class

              var cls = nodeClsMap.get(_node.type);
              ...
              var nodeData = nodesMap.get(_node.id); // 保存node data
              ...
              if ((_node$children = _node.children) != null && _node$children.length) {
                // 倒序遍历可以保证子节点的初始化在父节点之前
                var children = _node.children.map(child => nodeIdInstanceMap.get(child.id));

                instance.setChildren(children);
              }

              nodeIdInstanceMap.set(_node.id, instance); // 最后一项是根节点
