    ...
    execute: function () {
      ...
      _export("BehaviorManager", BehaviorManager = (_dec = ccclass("BehaviorManager"), _dec(_class = (_class2 = class BehaviorManager extends Component {
        ...
        disableBehavior(behavior, isPause = false) {
          ...
          const index = this.behaviorTrees.findIndex(tree => tree === behaviorTree); // 数组删除该树
          ...
          for (const behaviorTree of this.behaviorTrees) {
            ...
            for (let i = behaviorTree.activeStack.length - 1; i >= 0; i--) {
              ...
              while (prevStatus !== NodeStatus.Running && i < behaviorTree.activeStack.length && curStack.length) {
                const curIndex = curStack[curStack.length - 1]; // 记录前后两次，防止repeater子节点状态为success时，在一个tick里重复运行
        ...
        runNode(behaviorTree, index, stackIndex, prevStatus) {
          const node = behaviorTree.nodeList[index]; // 保证运行的节点在栈顶
        ...
        runParentNode(behaviorTree, index, stackIndex, status) {
          const node = behaviorTree.nodeList[index]; //防止running状态的并行节点重复运行
          ...
          while (node.canExecute() && (childStatus !== NodeStatus.Running || node.canRunParallelChildren())) {
            ...
            let curIndex = childIndex; // 防止repeater或untilSuccess等可以重复运行的节点，canExecute一直是true，会导致一直进入while
        ...
        pushNode(behaviorTree, index, stackIndex) {
          const stack = behaviorTree.activeStack[stackIndex]; // 防止重复推入
        ...
        popNode(behaviorTree, index, stackIndex, status, popChildren = true) {
          ...
          if (parentIndex !== -1) {
            if (node instanceof Condition) {
              ...
              if (parentCompositeIndex !== -1) {
                const composite = behaviorTree.nodeList[parentCompositeIndex]; //自己是条件节点并且父级的中断类型存在，创建重判断实例

                if (composite.abortType !== AbortType.None) {
                  // 父composite为LowerPriority时，条件重评估不希望此时执行，而是父composite popNode出去再执行
                  const compositeIndex = composite.abortType === AbortType.LowerPriority ? -1 : parentCompositeIndex; // 防止该条件存在条件重评估对象
        ...
        reevaluateConditionalNode(behaviorTree) {
          ...
          for (let i = behaviorTree.conditionalReevaluate.length - 1; i >= 0; i--) {
            ...
            for (let j = behaviorTree.activeStack.length - 1; j >= 0; j--) {
              ...
              const commonParentIndex = this.findCommonParentIndex(behaviorTree, curNodeIndex, index); // 该条件重评估的compositeIndex是commonParent的父级
            ...
            const compositeNode = behaviorTree.nodeList[behaviorTree.parentCompositeIndex[index]]; // 遍历左侧的条件重评估对象
            ...
            for (let j = i - 1; j >= 0; j--) {
              ...
              if (behaviorTree.parentCompositeIndex[conditionalReevaluate.index] === behaviorTree.parentCompositeIndex[index]) {
                ...
                if (compositeNode.abortType === AbortType.LowerPriority) {
                  conditionalReevaluate.compositeIndex = -1; // composite节点是Both或者Self，条件重评估的compositeIndex等于parentCompositeIndex
            ...
            conditionalParentIndex.push(compositeIndex); //从顶部到底部的顺序执行
        ...
        emitEvent(behaviorTree, nodeData, lifeCycle) {
          ...
          const curNode = behaviorTree.behavior.node; // 先从当前节点开始层序遍历

          let target = bfsCcNode(curNode, nodeUuid); // 找不到再从scene节点开始层序遍历
          ...
          const res = method.call(comp, methodData); // 用户忘记写返回值，一律返回失败
